...
### GitHub Rules (ALWAYS FOLLOW):
- User says "repos", "repositories", "GitHub", "issues", "PR", "code" → MUST call a github_* tool
- "List my repos" → call github_search_repositories
- "Create an issue" → call github_create_issue
...
### Slack History:
- Use search_knowledge_base for past
- Use get_channel_history for recent
...
/**
 * Slack tool definitions (UNCHANGED)
 */
const SLACK_TOOLS: OpenAI.Chat.ChatCompletionTool[] = [
  {
    type: 'function',
...
async function executeTool(
  ...
  try {
    switch (name) {
      case 'search_knowledge_base': {
        const results = await retrieve(args.query as string, {});
